...
FROM game,goal
WHERE goal.player LIKE 'Mario%' AND game.id = goal.matchid
...
SELECT player
FROM game,goal
WHERE game.stadium = 'National Stadium, Warsaw' AND game.id = goal. matchid
...
FROM game, goal 
WHERE matchid = id AND (team1 = 'GER' OR team2='GER') AND teamid != 'GER'
...
FROM game, goal 
WHERE (team1 = 'POL' OR team2 = 'POL') AND matchid = id 
...
SELECT matchid,mdate, COUNT(player)
FROM game, goal 
WHERE (team1 = 'GER' OR team2 = 'GER') AND matchid = id AND teamid = 'GER' 
GROUP BY matchid,mdate
...
SELECT mdate,team1,
       ...
       team2,
       SUM(CASE WHEN teamid = team2 THEN 1 ELSE 0 END) AS score2 FROM game, goal
WHERE id = matchid
